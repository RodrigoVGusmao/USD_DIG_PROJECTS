	...
	private void createContents() {
		...
		btnJogar.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				EstadoJogo estado = (EstadoJogo) velha.open();
				if(jogoNormal)
				{
					try
					{
						Banco banco = Banco.criarBanco();
						banco.inserirJogo(estado.estado, estado.jogo);
					}catch(SQLException e1)
					{
						JOptionPane.showMessageDialog(null, "Não foi possivel fazer salvar jogo!" + "\nErro: " + e1.getMessage(),
