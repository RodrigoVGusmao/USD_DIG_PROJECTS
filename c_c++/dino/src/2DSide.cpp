...
void staticObject::getCollisionState(side::element object)
{
    ...
    SDL_Rect intersectSec ={0,0,0,0};

    collision = SDL_IntersectRect(&objectRect, &thisRect, &intersectSec);

    if(collision)
    {
        float objSin = (object.position.y-object.oldPosition.y)/hypot(object.position.x-object.oldPosition.x, object.position.y-object.oldPosition.y);
        float objCos = (object.position.x-object.oldPosition.x)/hypot(object.position.x-object.oldPosition.x, object.position.y-object.oldPosition.y);
